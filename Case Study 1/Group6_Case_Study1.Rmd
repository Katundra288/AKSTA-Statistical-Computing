...
x <- seq(1, 2000, by = 100)
y <- gamma_fct(x)
plot(x, y, type = "l", col = "blue", lwd = 2, main = "Gamma Function", xlab = "x", ylab = "gamma_fct(x)")

```

We can try for several lower n and see that from around 340 on we only get NaNs. This might happen due to numerical precision issues computing the gamma function for very large values (given gamma(n) = (n-1)!). Useing the lgamma function might help overcoming this issue. It returns the natural logarithm of the absolute value of the gamma function. We would only need to use the exponential function for the final result.
...
plot(x, y, type = "l", col = "blue", lwd = 2, main = "Gamma Function", xlab = "x", ylab = "rho_n/sqrt(n)")
```
...
I would put everything in a loop.
...
kwtest(x, g, ...):
    ...
    Calculate the Kruskal-Wallis test statistic
      ...
      - Use the formula: STATISTIC = (12 * sum(group_sums^2) / (n * (n + 1)) - 3 * (n + 1)) / (1 - sum(TIES^3 - TIES) / (n^3 - n))
...
kwtest_fast <- function(x, g) {
  ...
  # Compute the sum of squared ranks for each group
  group_sums <- tapply(r, g, sum)
  
  # Compute the number of ties
  TIES <- table(x)
  
  # Calculate the Kruskal-Wallis test statistic
  # We optimize the code structure and unnecessary operations
  STATISTIC <- (12 * sum(group_sums^2) / (n * (n + 1)) - 3 * (n + 1)) / 
                (1 - sum(table(x)^3 - table(x)) / (n^3 - n))
  ...
  # Return the test statistic, degrees of freedom, and p-value
...
kruskal_wallis_test <- function(X, g) {
  # Initialize vector to store test statistics
